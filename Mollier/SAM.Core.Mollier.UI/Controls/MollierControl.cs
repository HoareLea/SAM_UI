...
namespace SAM.Core.Mollier.UI.Controls
{
    public partial class MollierControl : UserControl
    {
        ...
        private void createDiagramTemperatureLines(ChartType chartType, int temperature_Min, int temperature_Max, double humidityRatio_Min, double humidityRatio_Max, double pressure)
        {
            Dictionary<double, List<MollierPoint>> dictionary = null;
            if (chartType == ChartType.Mollier)
            {
                dictionary = Mollier.Query.ConstantDiagramTemperaturePoints(temperature_Min, temperature_Max, pressure, humidityRatio_Min, humidityRatio_Max);
            }
            else
            {
                dictionary = new Dictionary<double, List<MollierPoint>>();
                for (int i = temperature_Min; i <= temperature_Max; i++)
                {
                    ...
                    MollierPoint mollierPoint_2 = new MollierPoint(i, Mollier.Query.SaturationHumidityRatio(i, pressure) , pressure);

                    mollierPoints.Add(mollierPoint_1);
                    mollierPoints.Add(mollierPoint_2);
                    dictionary.Add(i, mollierPoints);
                }
            }
            ...
            {
                Series series = MollierChart.Series.Add("Temperature = " + keyValuePair.Key.ToString() + "[C]");
                series.IsVisibleInLegend = false;
                series.ChartType = SeriesChartType.Spline;

                ChartParameterType chartParameterType = keyValuePair.Key % 10 != 0 ? ChartParameterType.Line : ChartParameterType.BoldLine;

                series.Color = mollierControlSettings.VisibilitySettings.GetColor(mollierControlSettings.Color, chartParameterType, ChartDataType.DiagramTemperature);
                series.BorderWidth = chartParameterType == ChartParameterType.Line ? 1 : 2;

                foreach (MollierPoint mollierPoint in keyValuePair.Value)
                {
                    series.Points.AddXY(mollierPoint.HumidityRatio * 1000, mollierPoint.DryBulbTemperature);
                }
            }
        ...
        private void createRelativeHumidityLines_Mollier(int temperature_Min, int temperature_Max, double relative_humidity, double pressure)
        {
            ...
            for (int i = 1; i <= 10; i++)
            {
                ...
                for (int j = temperature_Min; j <= temperature_Max; j++)
                {
                    ...
                    double diagram_temperature = j; //Mollier.Query.DiagramTemperature(j, humidity_ratio, pressure);
        ...
        private void createRelativeHumidityLines_Psychrometric(int temperature_Min, int temperature_Max, double relative_humidity, double pressure)
        {
            for (int i = 1; i <= 10; i++)
            {
                string unit = (i * 10).ToString() + '%';
        ...
        private void createDensityLines(ChartType chartType, double density_Min, double density_Max, double pressure)
        {
        ...
        private void createEnthalpyLines(ChartType chartType, double enthalpy_Min, double enthalpy_Max, double pressure)
        {
        ...
        private void createWetBulbTemperatureLines(ChartType chartType, double temperature_Max, double wetBulbTemperature_Min, double wetBulbTemperature_Max, double pressure)
        {
        ...
        private void createSpecificVolumeLines(ChartType chartType, double specific_volume_Min, double specific_volume_Max, double pressure)
        {
        ...
        private Series CreateSeries(List<MollierPoint> mollierPoints, ChartType chartType, ChartDataType chartDataType, double value, string unit, string prefix)
        {
            ...
            foreach (MollierPoint mollierPoint in mollierPoints)
            {
                ...
                if (chartType == ChartType.Mollier)
                {
                    temperature = mollierPoint.DryBulbTemperature; //Mollier.Query.DiagramTemperature(mollierPoint);
                    if (double.IsNaN(temperature) || double.IsInfinity(temperature))
                    {
                        continue;
                    }
        ...
        private void add_MollierZones(ChartType chartType)
        {
            foreach (MollierZone mollierZone in mollierZones)
            {
                ...
                double labelY = chartType == ChartType.Mollier ? label.DryBulbTemperature : label.HumidityRatio;
        ...
        private void add_MollierPoints(ChartType chartType)
        {
            ...
            foreach (UIMollierPoint uIMollierPoint in mollierPoints)
            {
                ...
                double diagramTemperature = mollierPoint.DryBulbTemperature; //Mollier.Query.DiagramTemperature(mollierPoint);
        ...
        private void add_MollierProcesses(ChartType chartType)
        {
            ...
            for (int i = 0; i < mollierProcesses.Count; i++)
            {
                ...
                index = chartType == ChartType.Mollier ? series.Points.AddXY(start.HumidityRatio * 1000, start.DryBulbTemperature) : series.Points.AddXY(start.DryBulbTemperature, start.HumidityRatio);
                ...
                index = chartType == ChartType.Mollier ? series.Points.AddXY(end.HumidityRatio * 1000, end.DryBulbTemperature) : series.Points.AddXY(end.DryBulbTemperature, end.HumidityRatio);
                ...
                if (mollierProcess is CoolingProcess)
                {
                    ...
                    double Y = chartType == ChartType.Mollier ? apparatusDewPoint.DryBulbTemperature : apparatusDewPoint.HumidityRatio;
        ...
        private void createSeries_DewPoint(MollierPoint mollierPoint_1, MollierPoint mollierPoint_2, IMollierProcess mollierProcess, ChartType chartType, Color color, int borderWidth, ChartDashStyle borderDashStyle)
        {
            ...
            if (chartType == ChartType.Mollier)
            {
                ...
                series.Points.AddXY(mollierPoint_1.HumidityRatio * 1000, mollierPoint_1.DryBulbTemperature);
                series.Points.AddXY(mollierPoint_2.HumidityRatio * 1000, mollierPoint_2.DryBulbTemperature);
            }
            else
            {
        ...
        private void createSeries_ProcessesPoints(MollierPoint mollierPoint, UIMollierProcess UI_MollierProcess, ChartType chartType, string toolTipName = null, string pointType = "Default")
        {
            ...
            int index = chartType == ChartType.Mollier ? series.Points.AddXY(mollierPoint.HumidityRatio * 1000, mollierPoint.DryBulbTemperature) : series.Points.AddXY(mollierPoint.DryBulbTemperature, mollierPoint.HumidityRatio);
            switch (pointType)
            {
                case "Default":
                    series.MarkerSize = 8;
        ...
        private void createSeries_RoomProcess(UIMollierProcess uIMollierProcess)
        {
            ...
            index = mollierControlSettings.ChartType == ChartType.Mollier ? series.Points.AddXY(start.HumidityRatio * 1000, start.DryBulbTemperature) : series.Points.AddXY(start.DryBulbTemperature, start.HumidityRatio);
            series.Points[index].Tag = start;
            ...
            index = mollierControlSettings.ChartType == ChartType.Mollier ? series.Points.AddXY(end.HumidityRatio * 1000, end.DryBulbTemperature) : series.Points.AddXY(end.DryBulbTemperature, end.HumidityRatio);
            series.Points[index].Tag = end;
            ...
            double Y = mollierControlSettings.ChartType == ChartType.Mollier ? end.DryBulbTemperature : end.HumidityRatio;
        ...
        private void createPorcessesLabels_New_2(List<Tuple<MollierPoint, string>> points_list)
        {
            ...
            for (int i = 0; i < created_points.Count; i++)
            {
                ...
                double Y = mollierControlSettings.ChartType == ChartType.Mollier ? created_points[i].Item1.DryBulbTemperature : created_points[i].Item1.HumidityRatio;
        ...
        private void generateGraph_Mollier()
        {
            ...
            createRelativeHumidityLines_Mollier(System.Convert.ToInt32(temperature_Min), System.Convert.ToInt32(temperature_Max), relative_humidity, pressure);
            createDiagramTemperatureLines(ChartType.Mollier, System.Convert.ToInt32(temperature_Min), System.Convert.ToInt32(temperature_Max), humidityRatio_Min, humidityRatio_Max, pressure);


            //CREATING DENSITY LINE
            if (density_line)
                createDensityLines(ChartType.Mollier, density_Min, density_Max, pressure);
            //CREATING ENTHALPY LINE
            if (enthalpy_line)
                createEnthalpyLines(ChartType.Mollier, enthalpy_Min, enthalpy_Max, pressure);
            //CREATETING WET BULB TEMPERATURE LINE
            if (wet_bulb_temperature_line)
                createWetBulbTemperatureLines(ChartType.Mollier, temperature_Max, wetBulbTemperature_Min, wetBulbTemperature_Max, pressure);
            //CREATING SPECIFIC VOLUME LINE
            if (specific_volume_line)
                createSpecificVolumeLines(ChartType.Mollier, specific_volume_Min, specific_volume_Max, pressure);
            //CREATING P_w AXIS
        ...
        private void generateGraph_Psychrometric()
        {
            ...
            //create_relative_humidity_line_New(System.Convert.ToInt32(temperature_Min), System.Convert.ToInt32(temperature_Max), pressure);
            createRelativeHumidityLines_Psychrometric(System.Convert.ToInt32(temperature_Min), System.Convert.ToInt32(temperature_Max), relative_humidity, pressure);

            //CREATING DENSITY LINE
            if (density_line)
                createDensityLines(ChartType.Psychrometric, density_Min, density_Max, pressure);
            //CREATING ENTHALPY LINE
            if (enthalpy_line)
                createEnthalpyLines(ChartType.Psychrometric, enthalpy_Min, enthalpy_Max, pressure);
            //CREATING WET BULB TEMPERATURE LINE
            if (wet_bulb_temperature_line)
                createWetBulbTemperatureLines(ChartType.Psychrometric, temperature_Max, wetBulbTemperature_Min, wetBulbTemperature_Max, pressure);
            //CREATING SPECIFIC VOLUME LINE
            if (specific_volume_line)
                createSpecificVolumeLines(ChartType.Psychrometric, specific_volume_Min, specific_volume_Max, pressure);
        ...
        public void ColorPoints(bool generate, double percent, string chartDataType)
        {
            ...
            switch (chartDataType)
            {
                case "Temperature":
                    ...
                    double Y_Temperature = mollierControlSettings.ChartType == ChartType.Mollier ? uIMollierPoint_Temperature.MollierPoint.DryBulbTemperature : uIMollierPoint_Temperature.MollierPoint.HumidityRatio;
                ...
                case "Enthalpy":
                    ...
                    double Y_Enthalpy = mollierControlSettings.ChartType == ChartType.Mollier ? mollierPoint_Enthalpy.DryBulbTemperature : mollierPoint_Enthalpy.HumidityRatio;
